...
namespace MessageGraphicsDevice {
  ...
  public:
    std::string type; // png or svg
    std::string name; // graphics name
  };
  ...
  void SetDeviceInfo(BERTBuffers::GraphicsCommand *graphics, pDevDesc dd) {
  ...
  SEXP CreateMessageDevice(const std::string &name, const std::string &background, double width, double height, double pointsize, const std::string &type, void * pointer) {
    ...
    std::string normalized_type = "svg";
    if (!type.compare("png")) normalized_type = "png";

    MessageDeviceData *device_data = new MessageDeviceData;
    ...
    */
    ...
    dd->deviceSpecific = device_data;

    //std::string name = "BERT Message Device (";
